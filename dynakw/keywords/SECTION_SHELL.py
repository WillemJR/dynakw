...
class SectionShell(LSDynaKeyword):
    ...
    def __init__(self, keyword_name: str, raw_lines: List[str] = None):
        ...
        if self.keyword_type not in [KeywordType.SECTION_SHELL]:
            pass
    ...
    def _parse_raw_data(self, raw_lines: List[str]):
        ...
        card1_data[1] = int(card1_data[1]) if card1_data[1] is not None else None  # ELFORM
        card1_data[3] = int(card1_data[3]) if card1_data[3] is not None else None  # NIP
        card1_data[5] = int(card1_data[5]) if card1_data[5] is not None else None  # QR/IRID
        card1_data[6] = int(card1_data[6]) if card1_data[6] is not None else None  # ICOMP
        card1_data[7] = int(card1_data[7]) if card1_data[7] is not None else None  # SETYP

        self.cards["Card 1"] = {col: np.array([val], dtype=object) for col, val in zip(card1_columns, card1_data)}
        line_idx += 1
        ...
        # Convert to correct types
        card2_data[6] = int(card2_data[6]) if card2_data[6] is not None else None  # IDOF
        card2_data[7] = int(card2_data[7]) if card2_data[7] is not None else None  # EDGSET

        self.cards["Card 2"] = {col: np.array([val], dtype=object) for col, val in zip(card2_columns, card2_data)}
        line_idx += 1
        ...
        if icomp == 1 and nip is not None and nip > 0:
            ...
            if all_b_values:
                self.cards["Card 3"] = {col: np.array([val], dtype=object) for col, val in zip(b_cols, all_b_values)}
    ...
    def write(self, file_obj: TextIO):
        ...
        if card1 is not None:
            cols = ["SECID", "ELFORM", "SHRF", "NIP", "PROPT", "QR/IRID", "ICOMP", "SETYP"]
        ...
        for card_name, cols, types, num_fields in [
            ("Card 4a", ["DX", "DY", "ISPLINE", "IDILA", "IEBT", "IDIM"], ["F", "F", "I", "I", "I", "I"], 8),
            ("Card 4b", ["ITHELFM"], ["I"], 8),
            ("Card 4c", ["CMID", "BASELM", "DOMINT", "FAILCR", "PROPCR", "FS", "LS/FS1", "NC/CL"], ["I", "I", "I", "I", "I", "F", "F", "F"], 8),
            ("Card 4d", ["THKSCL"], ["F"], 8)
        ]:
            card = self.cards.get(card_name)
            if card is not None:
                line_parts = [self.parser.format_field(card.get(col, [None])[0], typ) for col, typ in zip(cols, types)]
                line_parts.extend([self.parser.format_field(None, "F")] * (num_fields - len(line_parts)))
                file_obj.write("".join(line_parts) + "\n")
        ...
        if card5 is not None:
            ...
            types = ["I"] * 8
            line_parts = [self.parser.format_field(card5.get(col, [None])[0], typ) for col, typ in zip(cols, types)]
            file_obj.write("".join(line_parts) + "\n")
            ...
            if card51 is not None:
                ...
                for i in range(nrows):
                    line_parts = [self.parser.format_field(card51.get(col, [None]*nrows)[i], typ) for col, typ in zip(cols, types)]
                    line_parts.extend([self.parser.format_field(None, "F")] * 5)
                    file_obj.write("".join(line_parts) + "\n")
