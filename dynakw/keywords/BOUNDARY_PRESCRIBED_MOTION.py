...
class BoundaryPrescribedMotion(LSDynaKeyword):
    ...
    keyword_aliases = ["*BOUNDARY_PRESCRIBED_MOTION_NODE",
    ...
    def _parse_raw_data(self, raw_lines: List[str]):
        ...
        for line in card_lines:
            parsed_fields = self.parser.parse_line(
                line, field_types, long_format=False)
        ...
        card_dict = {col: np.array(
            [row[i] for row in parsed_data], dtype=object) for i, col in enumerate(columns)}
        ...
        for idx in range(data_length):
            ...
            for i, col in enumerate(columns):
                ...
                field_str = self.parser.format_field(
                    value, field_types[i], long_format=False)
