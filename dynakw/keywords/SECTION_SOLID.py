...
class SectionSolid(LSDynaKeyword):
    ...
    def _parse_raw_data(self, raw_lines: List[str]):
        ...
        if "EFG" in options:
            ...
            self.cards["Card 2a.1"] = {col: np.array([val], dtype=object) for col, val in zip(card2a1_cols, card2a1_data)}
            line_idx += 1
            # Card 2a.2 (Optional)
            if line_idx < len(card_lines):
                ...
                if any(x is not None for x in card2a2_data):
                    self.cards["Card 2a.2"] = {col: np.array([val], dtype=object) for col, val in zip(card2a2_cols, card2a2_data)}
                    line_idx += 1
        elif "SPG" in options:
            # Card 2b.1
            ...
            self.cards["Card 2b.1"] = {col: np.array([val], dtype=object) for col, val in zip(card2b1_cols, card2b1_data)}
            line_idx += 1
            # Card 2b.2 (Optional)
            if line_idx < len(card_lines):
                ...
                if any(x is not None for x in card2b2_data):
                    self.cards["Card 2b.2"] = {col: np.array([val], dtype=object) for col, val in zip(card2b2_cols, card2b2_data)}
                    line_idx += 1
            ...
            if line_idx < len(card_lines):
                ...
                if any(x is not None for x in card2c_data):
                    self.cards["Card 2c"] = {col: np.array([val], dtype=object) for col, val in zip(card2c_cols, card2c_data)}
                    line_idx += 1
        ...
        if elform in [101, 102, 103, 104, 105]:
            ...
            self.cards["Card 3"] = {col: np.array([val], dtype=object) for col, val in zip(card3_cols, card3_data)}
            line_idx += 1
            ...
            if nip > 0:
                ...
                self.cards["Card 4"] = {col: arr[:, i] for i, col in enumerate(card4_cols)}
            ...
            if lmc > 0:
                ...
                if p_values_truncated:
                    self.cards["Card 5"] = {col: np.array([val], dtype=object) for col, val in zip(p_cols, p_values_truncated)}
