...
def create_unstructured_grid(dyna_file: DynaKeywordFile) -> pv.UnstructuredGrid:
    ...
    if shell_keywords:
        ...
        for i in range(len(n1)):
            ...
            if n4[i] > 0 and n4[i] != n3[i]:
                points_indices = [
                    node_id_to_point_index[n1[i]], node_id_to_point_index[n2[i]],
                    node_id_to_point_index[n3[i]], node_id_to_point_index[n4[i]]
                ]
    ...
    if solid_keywords:
        ...
        n5, n6, n7, n8 = get_node_col('N5'), get_node_col('N6'), get_node_col('N7'), get_node_col('N8')

        for i in range(num_elements):
            # Check for Hexahedron (8 nodes)
            if n8[i] > 0:
                points_indices = [node_id_to_point_index[nid] for nid in [n1[i], n2[i], n3[i], n4[i], n5[i], n6[i], n7[i], n8[i]]]
                cells_list.extend([8] + points_indices)
                cell_types_list.append(VTK_HEXAHEDRON)
            # Check for Wedge (6 nodes)
            elif n6[i] > 0:
                points_indices = [node_id_to_point_index[nid] for nid in [n1[i], n2[i], n3[i], n4[i], n5[i], n6[i]]]
                cells_list.extend([6] + points_indices)
                cell_types_list.append(VTK_WEDGE)
            # Check for Tetrahedron (4 nodes)
            else:
                points_indices = [node_id_to_point_index[nid] for nid in [n1[i], n2[i], n3[i], n4[i]]]
                cells_list.extend([4] + points_indices)
                cell_types_list.append(VTK_TETRA)

    if not cells_list:
        raise ValueError("No supported element types (*ELEMENT_SHELL, *ELEMENT_SOLID) found or parsed in the file.")
