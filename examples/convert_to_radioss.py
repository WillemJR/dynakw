...
def write_as_radioss( dkw:dynakw.DynaKeywordFile, out_fname:str ):
    """ Translate LS-DYNA keyword file to a Radioss one.

    Args:
      dkw (dynakw.DynaKeywordFile) :
      out_fname (str) : 
    """
        ...
        for kw in dkw.keywords():
            if kw.type == dynakw.KeywordType.NODE:
                ...
                for i in range(len(card['NID'])):
                    f.write(f"{card['NID'][i]} {card['X'][i]} {card['Y'][i]} {card['Z'][i]}\n")
            elif kw.type == dynakw.KeywordType.ELEMENT_SHELL:
                f.write("/SHELL\n")
                card = kw.cards['Card 1']
                for i in range(len(card['EID'])):
                    f.write(f"{card['EID'][i]} {card['PID'][i]} {card['N1'][i]} {card['N2'][i]} {card['N3'][i]} {card['N4'][i]}\n")
            elif kw.type == dynakw.KeywordType.ELEMENT_SOLID:
                f.write("/SOLID\n")
                card = kw.cards['Card 1']
                for i in range(len(card['EID'])):
                    f.write(f"{card['EID'][i]} {card['PID'][i]} {card['N1'][i]} {card['N2'][i]} {card['N3'][i]} {card['N4'][i]} {card['N5'][i]} {card['N6'][i]} {card['N7'][i]} {card['N8'][i]}\n")
                ...
                for i in range(len(card1['PID'])):
                    ...
                    f.write(f"{card2['PID'][i]} {card2['SECID'][i]} {card2['MID'][i]}\n")
            elif kw.type == dynakw.KeywordType.MAT_ELASTIC:
                f.write("/MAT/ELASTIC\n")
                card = kw.cards['card1']
                for i in range(len(card['MID'])):
                    f.write(f"{card['MID'][i]} {card['RO'][i]} {card['E'][i]} {card['PR'][i]}\n")
                ...
                for i in range(len(card1['SECID'])):
                    ...
                    f.write(f"{card2['T1'][i]} {card2['T2'][i]} {card2['T3'][i]} {card2['T4'][i]}\n")
